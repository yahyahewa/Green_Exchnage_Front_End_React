...
const SingleCard = () => {
  ...
  const { data: allProduct } = useGetProductsQuery({ search, page, category })
  ...
  return (
    <div className="my-10">
      <Banner search={search} setSearch={setSearch} category={category} setCategory={setCategory} searchHandler={()=>searchHandler()} />
          ...
          <Link to={`/products/${card._id}`} key={card._id}>
              ...
              <div className="flex my-4">
                ...
                <div className="flex flex-col ml-4">
                  <span className="font-bold">{card?.owner?.fullname}</span>
                     ...
                     <span className=" flex items-center text-sm "><IoLocationSharp className="mr-1 text-green" />{ card?.address}</span>
                  </div>
           ...
           <h1 className="font-bold text-lg text-gray-800 px-4 pb-2 ">
              ...
              <div className="flex  overflow-hidden relative">
                <img src="https://images.unsplash.com/photo-1556905055-8f358a7a47b2?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=870&q=80" alt="products"
